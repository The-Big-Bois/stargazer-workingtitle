...
class Player(Character):
    ...
    def item_pickup(self, item):       
        ...
        if item.item_type in cloaks:
            ...
            #if 
            #self.cloak_index += 1
            #if self.cloak_index == 0 and len(self.cloak_list) == 3:
             #   self.cloak_index += 2
...
class Enemy(Character):
    ...
    def __init__(self, x, y, walk_speed, air_speed, sprite_sheet_list, sprite_sheet_list_names, patrol, AI, player):
        super().__init__(x, y, walk_speed, air_speed, sprite_sheet_list, sprite_sheet_list_names)
    ...
    def update(self):
        ...
        pler = (self.player.rect.left - self.rect.right)
        elpr = (self.rect.left - self.player.rect.right)
        pteb = (self.player.rect.top - self.rect.bottom)
        etpb = (self.rect.top - self.player.rect.bottom)
        min_x = min(pler, elpr)
        min_y = min(pteb, etpb)
        ...
        if min_x > self.AI['aggro_x'] and min_y > self.AI['aggro_y']:
            self.detection = False
        ...
        if self.in_attack_range:
            ...
            target_hit = pygame.sprite.spritecollide(self.hitbox, self.player_hitbox_group, False)
            if len(target_hit) > 0:
